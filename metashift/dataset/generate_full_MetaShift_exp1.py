...
In addition, to save storage, all copied images are symbolic links. You can set `use_symlink=False` in the code to perform actual file copying. If you really want to generate the **full** MetaShift, then set `ONLY_SELECTED_CLASSES = True` in [dataset/Constants.py](./dataset/Constants.py). 
...
def load_candidate_subsets():
    pkl_save_path = "dataset/meta_data/full-candidate-subsets.pkl" 
...
##################################
def copy_image_for_subject(root_folder, subject_str, subject_data, node_name_to_img_id, trainsg_dupes, use_symlink=False):
...
def build_subset_graph(subject_most_common_list, node_name_to_img_id, trainsg_dupes, subject_str,seed):
    N_sets = len(subject_most_common_list)
    subject_most_common_list = list(subject_most_common_list)
    # print('N_sets', N_sets)
    Adjacency_matrix = np.ones((N_sets, N_sets))
    for i, ii in enumerate(subject_most_common_list):
        # print("ii: ",ii)
        for j,jj in enumerate(subject_most_common_list):
            # print("jj: ",jj)
            set_A = node_name_to_img_id[ii] - trainsg_dupes
            set_B = node_name_to_img_id[jj] - trainsg_dupes
            overlap_set = set_A.intersection(set_B)
            # print("overlap set length: ", len(overlap_set))
            # print("min set length: ", min(len(set_A), len(set_B)))
    ...
    # w,v = np.linalg.eigh(Adjacency_matrix)
    # print(w)
    ...
    #     print([len(i) for i in tuple(sorted(c) for c in communities)])
    # return G, Adjacency_matrix, labels, comms
    # return comms
    # print(pars)
    return G, pars_sc, Adjacency_matrix
    # return pars, Adjacency_matrix

def build_subset_graph_test(subject_most_common_list, node_name_to_img_id, trainsg_dupes, subject_str):
    N_sets = len(subject_most_common_list)
    subject_most_common_list = list(subject_most_common_list)
    # print('N_sets', N_sets)
    Adjacency_matrix = np.ones((N_sets, N_sets))
    ...
    # print("Kernighan_lin_bisection: ")
    # pars = nx_comm.kernighan_lin_bisection(G,max_iter=1,seed=1)
    # print(pars)
    # qual = nx_comm.modularity(G, pars)
    # print("quality: ", qual)
    # pars = nx_comm.kernighan_lin_bisection(G,max_iter=100,seed=1)
    # print(pars)
    # qual = nx_comm.modularity(G, pars)
    # print("quality: ", qual)

    # print("spectral clustering: ")
    # sc = SpectralClustering(n_clusters = 2, affinity = 'precomputed', assign_labels='discretize',n_init=1000)
    # sc.fit(Adjacency_matrix)
    # print(labels)
    # print(sc.labels_)
    # pars_sc = [1,1]
...
def preprocess_groups(output_files_flag=True, subject_classes = Constants.SELECTED_CLASSES):
    ...
    for subject_str, subject_data in subject_group_summary_list:
        ...
        if output_files_flag:
            ...
            copy_image_for_subject(META_DATASET_FOLDER, subject_str, subject_data, node_name_to_img_id, trainsg_dupes, use_symlink=False) # use False to share 
