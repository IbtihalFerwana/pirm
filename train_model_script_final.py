...
def mean_accuracy(logits, y):
    ...
    acc = (logits.argmax(dim=1) == y).int().float().sum()
    return acc    
    ...
    for epoch in range(epochs):
        ...
        for step in range(steps):
            ...
            if method == 'irm':
                ...
                train_penalty = torch.stack([envs[i]['penalty'] for i in range(d_num) if envs[i]['train']==True]).mean()
            ...
            if step % (steps//3) == 0 and (epoch+1)%args.epoch_print_step == 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_split', type=str, default='equal', help='data directory')
    parser.add_argument('--training_years', nargs='+', default = ['1980','1990','2000','2005','2010'], help='a list of partitioned periods, indicated by the starting year')
    parser.add_argument('--testing_years', nargs='+', default = ['1980','1990','2000','2005','2010'], help='a list of partitioned periods, indicated by the starting year')
    ...
    parser.add_argument('--save_best_model', type=bool, default=False, help='save best model weights based on avg validation acc')
    parser.add_argument('--epoch_print_step', type=int, default=2, help='epochs for printing model performance')
